...
def main():
    # Define the matrixi
    np.random.seed(0)
    input_embed = np.random.normal(size=(18, 100))
    linear_w = np.random.normal(size=(100, 100))
    linear_b = np.random.normal(size=100)
    output_embed = np.random.normal(size=(18, 100))
    output_embed_b = np.random.normal(size=18)
    ...
    luckydrawhist.close()     
    print "The (Blue Ball) Predict Result Is:"
    print predict_one(input_embed,linear_w,linear_b,output_embed, output_embed_b, 0)
